...
saveRDS(integrated_data, "output/seu.objs/study_integrations/harmony_fib.rds")
integrated_data = readRDS( "output/seu.objs/study_integrations/harmony_fib.rds")
...
integrated_data= subset(integrated_data,opt_clust_integrated==10 , invert= T )
...
weird_cluster= rownames(integrated_data@meta.data %>% filter(opt_clust_integrated== 6))
saveRDS(weird_cluster, "output/seu.objs/study_integrations/cellid_to_exclude.rds")
...
# seurat integration approach: ----------------------------------------------------------------


hfpef= FindVariableFeatures(hfpef)
x= FindIntegrationAnchors(objlist)
seu.combined= IntegrateData(x)

saveRDS(seu.combined, "output/seu.objs/study_integrations/fibs_.rds")
...
DimPlot(seu.combined, group.by = "orig.ident")
seu.combined[[]]
